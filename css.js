...
let html = [
    ...
    "Use indentation.<br><br> Space it out. <br><br>Add &lt;!-- Subheadings --&gt;.  <br><br>Use meaningful and description names for your elements, classes, and IDs.",
    "What is semantic HTML? Why is it used?",
    "A coding practice that involves using HTML elements that have a <b>clear meaning and purpose</b> based on their name, rather than using generic elements such as <div> or <span> to style content.<br><br> It’s better for accessibility, readability and SEO. More examples are: &ltheader&gt &ltsection&gt &ltfooter&gt &ltmain&gt.",
...
let css = [
    ...
    "In flexbox, how do you set a minimum size for an element while also allowing it to expand to fit its content when needed?",
    "To make a responsive layout in flexbox, you can use the min-height and min-width properties in combination with height: auto and width: auto.",
    "How should you go about styling buttons in CSS?",
    "Create a <b>.btn class</b> with all the core styling.<br><br> Create<b> .btn-primary</b>, <b>btn-success</b> etc. as modifiers for other buttons depending on the action they perform. <br><br>Add <b>:focus</b> state as well as <b>:hover</b> state in case a user selects it with their keyboard.",
    "What's the key difference between flexbox and CSS grid?",
    "Flexbox is a <b>one-dimensional layout</b> system that is best suited for arranging elements in a row or column. <br><br>CSS grid, on the other hand, is a <b>two-dimensional layout</b> system that allows you to define both rows and columns, and place elements into specific grid cells.",
    "How would you code a grid container with three columns and three rows, each with a width and height of 100px using CSS grid?",
    "display: grid;<br><br>  grid-template-columns: 100px 100px 100px;<br><br> grid-template-rows: 100px 100px 100px;<br><br>",
    "How do you place content within a CSS grid? ",
    "You can place content within a CSS grid by using the<b> grid-column and grid-row </b>properties to specify the start and end points of the content within the grid.<br><br> For example, to place the item within the 1st and 2nd columns of the grid (spanning two columns) and the 2nd and 3rd rows of the grid (spanning two rows):<br><br>.item {<br><br>  grid-column: 1 / 3; <br><br> grid-row: 2 / 4; <br><br> }",
    "How do you add space between the rows and columns in CSS grid? ",
    "With the <b>grid-gap</b> property",
    "How can you create a grid that has a fixed number of rows and automatically expands to fit the content in CSS grid?",
    ".container {<br><br> display: grid; <br><br> grid-template-columns: 1fr 1fr; <br><br> grid-template-rows: repeat(auto-fit, minmax(100px, auto)); <br><br> height: auto; <br><br> }",
    "What are the basic steps to code flexbox?",
    "Create a container element that will be the parent of the flex items. Set the display property of the container to flex to activate flexbox behavior. Define the flex direction of the container using the<b> flex-direction</b> property. Use the <b>justify-content</b> property to control the spacing between items along the main axis – the flex-direction. Use the <b>align-items</b> property to align the flex items on the other (cross) axis. Flexbox is super helpful because it allows you to dynamically alter the height/width of flex items to fill the available space, making it responsive. If you set <b>flex-grow </b>property to 1 on all of your items, they’ll grow to fill the available space at the same rate. Similarly, if you set <b>flow-shrink</b> property to 1 on all of your items, they’ll shrink at the same pace. If you’d rather them wrap onto another line instead of shrink, use the <b>flex-wrap</b> property.",
...
let random = [
    ...
    "Write <b>self-explanatory variable and function names</b> that accurately describe their purpose. <b>Keep functions short</b> and focused on doing one thing well. <b>Use consistent formatting and indentation </b>to make your code more readable.<br><br><b> Use comments </b>to explain complex logic or any unclear parts of your code. <b>Avoid unnecessary or redundant code</b>, such as unused variables or functions. Use descriptive and <b>clear error messages </b>to help with debugging.<br><br> Follow established naming conventions and patterns for variables, functions, and files.<b> Use whitespace effectively</b> to group related code and make it easier to read. <b>Write tests</b> to ensure that your code works as intended and to catch bugs early.",
    "How can we ensure accessibility?",
    "Use <b>semantic HTML tags</b> that accurately describe the content on the page, such as header, nav, main, article, section, and footer. Use<b> alt text </b>for images. Use appropriate color contrast. <br><br>Avoid using generic link text like ‘click here’ or ‘read more’. Instead, use <b>descriptive text</b> that accurately describes the link destination.<br><br> Use <b>ARIA</b> (Accessible Rich Internet Applications) attributes to provide additional context and information for users of assistive technologies. Provide<b> keyboard navigation</b>. Provide captions and transcripts for videos. Ensure form inputs are labelled. Avoid using flashing or blinking elements. Test with accessibility tools.",
